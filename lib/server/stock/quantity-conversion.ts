const unitGroups = {
  ...
  function getUnitGroup(unit: string) {
    ...
    for (const [group, units] of Object.entries(
      unitGroups
    )) {
      if (
        (units as readonly string[]).includes(
          normalizedUnit
        )
      ) {
        return group;
  ...
  export function areUnitsCompatible(
    ...
    const existingGroup = getUnitGroup(
      normalizedExisting
    );
  
    const incomingGroup = getUnitGroup(
      normalizedIncoming
    );
